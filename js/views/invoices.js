// ============================================================================
// invoices.js — invoices/bills with payment tracking, printable view, sharing.
// A document is addressed to a client (via its project) OR a vendor, and for a
// vendor it can be outgoing (they owe me) or incoming (I owe them / a bill).
// ============================================================================
...
async function renderList(outlet) {
  const [invoices, projects, vendors] = await Promise.all([
    db.list('invoices'), db.list('projects'), db.list('vendors'),
  ]);
  const list = invoices.sort(byNewest);
  const cur = currency();
  const live = list.filter((i) => !['Draft', 'Cancelled'].includes(i.status));
...
async function renderDetail(outlet, id) {
  ...
  const to = recipientOf(inv, projects, vendors);
  const cur = currency();
  const t = invoiceTotals(inv);
  const st = displayStatus(inv);
  const incoming = isIncoming(inv);
  ...
  outlet.innerHTML = `
    ...
    <div class="quote-top"><h1>${escapeHtml(inv.number || 'Invoice')}</h1>${statusChip(st, STATUS_CLASS)}${incoming ? '<span class="chip chip--warn">Bill — I owe</span>' : ''}</div>
    <div class="meta-line muted small">
      ${incoming ? 'From' : 'To'}: ${escapeHtml(to.name || '—')}${to.projectTitle ? ` · ${escapeHtml(to.projectTitle)}` : ''} · ${fmtDate(inv.date)}${inv.dueDate ? ` · due ${fmtDate(inv.dueDate)}` : ''}
    </div>
    ${inv.description ? `<div class="note-box">${escapeHtml(inv.description)}</div>` : ''}

    <div class="stat-grid">
      <div class="stat"><span class="stat__label">Total</span><span class="stat__val stat__val--sm">${money(t.total, cur)}</span></div>
      <div class="stat"><span class="stat__label">${incoming ? 'Paid out' : 'Received'}</span><span class="stat__val stat__val--sm">${money(t.paid, cur)}</span></div>
...
function recordPayment(inv) {
  ...
  const bal = invoiceTotals(inv).balance;
  const s = openSheet({
    title: isIncoming(inv) ? 'Record payment made' : 'Record payment',
    body: `<form id="f" class="form">
      <p class="muted small">Balance ${isIncoming(inv) ? 'to pay' : 'due'}: <strong>${money(bal, cur)}</strong></p>
      ${row(
...
export async function editInvoice(preset = {}) {
  ...
  const s = openSheet({
    ...
    body: `<form id="f" class="form">
      ${recipientFields(inv, projects, vendors, { withDirection: true })}
      ${row(
      ...
      ${field('Description', textarea('description', inv.description, { rows: 2, placeholder: 'Short summary shown under the title (optional)' }))}
      ...
      ${field('Notes / terms', textarea('notes', inv.notes, { placeholder: 'Payment terms, bank details…' }))}
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (isNew) {
      const stg = settings();
      number = `${stg.invoicePrefix || 'INV-'}${String(stg.invoiceSeq || 1).padStart(4, '0')}`;
      await updateSettings({ invoiceSeq: (stg.invoiceSeq || 1) + 1 });
    }
    const rec = { ...inv, ...base, ...party, items, number };
...
export async function invoiceFromQuote(q) {
  const stg = settings();
  const number = `${stg.invoicePrefix || 'INV-'}${String(stg.invoiceSeq || 1).padStart(4, '0')}`;
  await updateSettings({ invoiceSeq: (stg.invoiceSeq || 1) + 1 });
  const inv = {
    partyType: q.partyType || 'client', projectId: q.projectId, vendorId: q.vendorId,
    partyName: q.partyName, direction: 'out',
    items: (q.items || []).map((i) => ({ ...i })),
    discount: q.discount, taxRate: q.taxRate,
    description: q.description, notes: q.notes || stg.invoiceTerms || '',
    date: todayISO(), dueDate: new Date(addDays(new Date(), 15)).toISOString().slice(0, 10),
...
// ---- share ----------------------------------------------------------------
function shareInvoice(inv, to) {
  const cur = currency();
  const t = invoiceTotals(inv);
  const stg = settings();
  const lines = [
    `${stg.businessName || 'Studio'} — Invoice ${inv.number}`,
    to.name ? `For: ${to.name}` : '',
  ...
  shareText({ title: `Invoice ${inv.number}`, text: lines, contact: to });
}

// ---- printable view -------------------------------------------------------
function printInvoice(inv, to) {
  const cur = currency();
  const stg = settings();
  const t = invoiceTotals(inv);
  const incoming = isIncoming(inv);
    ...
    <div class="pq">
      <div class="pq__head">
        <div class="pq__brandwrap">
          ${stg.logo ? `<img class="pq__logo" src="${escapeHtml(stg.logo)}" alt="">` : ''}
          <div>
            <div class="pq__brand">${escapeHtml(stg.businessName || 'Studio')}</div>
            <div class="pq__muted">${escapeHtml(stg.address || '')}</div>
            <div class="pq__muted">${escapeHtml([stg.phone, stg.email].filter(Boolean).join(' · '))}</div>
          </div>
        </div>
        <div class="pq__title">${incoming ? 'BILL' : 'INVOICE'}</div>
      </div>
      <div class="pq__meta">
        <div><strong>${escapeHtml(inv.number || '')}</strong><br>Date: ${fmtDate(inv.date)}${inv.dueDate ? `<br>Due: ${fmtDate(inv.dueDate)}` : ''}</div>
        <div class="pq__to">
          <div class="pq__muted">${incoming ? 'From' : 'Bill to'}</div>
      ...
      ${inv.description ? `<p class="pq__desc">${escapeHtml(inv.description)}</p>` : ''}
      ...
      <div class="pq__foot">Thank you. — ${escapeHtml(stg.businessName || 'Studio')}</div>
    </div>`;
